...
// 工程打开事件 → 形态策略：新建/打开任何工程都回到完整演播厅产品形态；
// 切到工作台时，右下角缩略视窗已停靠可用（点缩略窗随时回完整演播厅）。
...
export class DawdexProjectModeController {
    ...
    readonly #initialWorkbenchDeepLink: boolean

    #hadProject = false
    #openedProjectCount = 0

    constructor(session: DawdexUiSession, initialWorkbenchDeepLink: boolean = false) {
        this.#session = session
        this.#initialWorkbenchDeepLink = initialWorkbenchDeepLink
    }
    ...
    update(hasProject: boolean, suppressSwitch: boolean = false): void {
        ...
        if (!opened) {return}
        const startInWorkbench = this.#openedProjectCount === 0 && this.#initialWorkbenchDeepLink
        this.#openedProjectCount += 1
        if (suppressSwitch) {return}
        this.#session.setViewMode(startInWorkbench ? "workbench" : "product")
    }
}
