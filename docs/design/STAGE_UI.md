...
> 维护：成员 A（UI 与舞台层）· 最近更新：2026-07-25（PR #12，角色入场 + 电梯 + 六房间巡棚 + 工作台）
...
3. **写实比例角色**（v2 立绘）：5 头身侧视、细像素颗粒、暖金轮廓光，
   ...
   真实轨道角色。
4. **巡棚频道**（§11.1 落地）：外壳顶栏 `CH ‹ 房间名 ›` 切台，硬切无
   过渡（TV 气质）。六个频道：演播大厅（视频）/ 鼓棚 / 吉他贝斯棚 /
   键盘阁楼 / 控制室 / 休息室。每个房间只显示本房间的演员
   （`.stage[data-room]` 演员表，站位按各房间场景标定）；制作人常驻
   控制室（非轨道角色，不参与五态机与入场）。支持 `?room=<id>` 深链，
   演示导航用。弹幕、走带、REC 灯牌、雪花噪点为全局层，跨房间一致。
...
- 顶栏保留为右缘侧拉抽屉，显示品牌、ON AIR、频道和最近事件；
- `RealUiEventBridge` 仍按 500 ms 同步，返回演播厅即显示最新状态；
