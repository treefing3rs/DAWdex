...
> 版本：v0.2（AdventureX 2026 Hackathon Demo）
> 日期：2026-07-23
> 产品名：**DAWdex**
> 首个支持的 DAW：Ableton Live 12
> 内部交付截止：**2026-07-25 22:00**（7 月 26 日仅保留提交与现场缓冲）
...
- Ableton 连接自检；
- 读取当前工程和 Track；
- 一个能跑通的 Agent Runtime Adapter；Runtime 尚未最终选型时，优先接入最先通过
  Spike 的现成开源 Agent，不同时维护两个实现；
- 一个 Music Director 主 Agent；
- 一个固定高质量修改任务的结构化意图与计划；
- Apply 前确认；
- Ableton 写操作串行执行；
- 至少能创建或修改 Track / Clip / MIDI 中的一条真实链路；
- 核心结果读回验证；
- 最小 Action Log：计划、工具、结果；
- 同一小节的 Before / After；
- 固定 Demo 工程；
- 一键或明确的 Demo 重置流程；
- 90 秒稳定演示。

### P0-Track：申报 08 B 站赛道时必须完成

- 3–5 条预设弹幕；
- 固定窗口聚合为一个 `UserMusicRequest`；
- 复用 DAWdex 主执行链，不直接从弹幕调用 MCP；
- 创作者确认后再执行；
- 至少展示一次弹幕驱动的可编辑 Ableton 修改。
...
- 7 月 24 日中午前必须完成 `Electron → Agent → MCP → Ableton` 的最短链路；
- 7 月 24 日结束前必须完成一次 UI 内发起、Ableton 内生效的可验证修改；
- 7 月 25 日 15:00 功能冻结；
- 7 月 25 日 18:00 之后只修阻塞 Demo 的问题；
- 7 月 25 日 22:00 前完成提交材料、演示视频和备份；
- 如果 P0 主链不稳定，立即砍掉真实多 Agent、持久化、复杂 MIDI Diff 和完整弹幕接入。
...
- 聚合成一次受控修改；
- 由创作者确认；
- 展示弹幕将复用同一条执行链，现场条件允许时触发一次小范围变化；
- 强调弹幕是 DAWdex 的一个输入 Skill，不是产品主页。

不申报 B 站赛道时，这 12 秒改为下一条自然语言追问。
...
赛道原文、选择条件和可直接提交的回答见
[DAWdex 参赛赛道策略与提交回答](track-strategy.md)。

### 选择结论

| 优先级 | 赛道 | 结论 |
|---|---|---|
| 主选 | 08｜干杯！创造属于你的 B 站新纪元 | 方向一；以弹幕编曲 Skill 回应赛道，但产品本体仍是 DAW Agent |
| 次选 | 05｜不做第一，做唯一 | 以音乐工作流、MCP 实践和三人互补能力回应“为什么是你们” |
| 可选 | 07｜小红书 Build in Public | 仅在真实发布构建记录并吸收社区反馈后选择 |
| 条件性 | 04｜Qoder | 只有实际使用 Qoder 完成核心开发时选择 |
| 条件性 | 17｜Superun | 只有 Superun 成为主体逻辑核心工具时选择 |

当前使用 Codex 跑通 MCP 并不能自动满足 Qoder 或 Superun 的参赛要求。不得把
“产品理念匹配”写成“已使用指定工具”。
...
> DAWdex 让弹幕不再只是作品结束后的反馈，而能在创作者确认后成为下一版可编辑
> 编曲的输入。

### 对 05 唯一性赛道的一句话

> 我们不是从 Agent 技术出发硬找一个音乐场景，而是从真实 DAW 修改痛点出发，
> 已经把 Coding Agent、MCP 与 Ableton Live 跑到了一起。
...
比赛 Demo 允许按 P0 降级，但长期产品 v0.1 只有在以下条件满足时完成：
...
在 2026-07-25 22:00 前同时满足：

- P0 主链连续成功三次；
- 08 赛道演示 Skill 至少成功一次；
- 有可用安装包或固定开发环境启动脚本；
- 有 90 秒视频、备用录屏、截图、架构图和团队分工；
- 赛道回答不包含未发生的用户反馈或未使用的赞助方工具；
- 三位成员都能独立讲清产品一句话、Demo 顺序和失败兜底。
